   ...
   if (typeof define === "function" && define.amd) {
    ...
    Backbone.sync = function(method, model, options) {
        ...
        if (useXDomainRequest && !Backbone.$.support.cors) {
            ...
            console.info("Using CORS as browser not supporting CrossDomain")
            ...
            /* MY HACK BEGINS 
                NOT SURE WHY THIS ISNT PRODUCED NORMALLY
            */
            params.data= "";        
            for (var m in options.data)
            {
                if (params.data!="")
                {
                    params.data+="&"
                }
                params.data += m+"="+options.data[m];
